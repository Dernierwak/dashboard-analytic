-- ============================================================================
-- LA FILE « RECOS NEWS » — classificateur du Graphe A (compte entier)
...
-- (`ai_reco`) à quelle catégorie de `saas/recos_ia/reco_engine.py` elle
-- correspond — parmi des valeurs fermées, ou explicitement "aucune". Quand
-- aucune catégorie ne correspond, la piste ne doit jamais retomber dans un
-- fourre-tout générique « autre » (même défaut déjà corrigé côté recos par
-- thème, en faisant déclarer la catégorie plutôt que la deviner après coup) :
-- elle part ici, dans une file consultable.
--
-- CE QUE CETTE TABLE N'EST PAS : un pipeline de décision. Personne ne sait
-- aujourd'hui définir « assez de fois pour devenir une catégorie » ni sur
-- quelle population, et le volume de comptes est trop faible pour que ce soit
-- utile. Cette table n'a donc ni statut, ni compteur, ni promotion
-- automatique : c'est un endroit à consulter à la main (Supabase Studio),
-- pour décider un jour si un motif répété mérite une catégorie codée en dur
-- dans `reco_engine.py`. Aucune page Pulse dédiée pour l'instant — un
-- chantier séparé si besoin plus tard.
--
