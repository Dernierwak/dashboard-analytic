...
| `labels-couverture` (rang 6) | le rang 6 était une barre de progression linéaire (« 100 % rattaché à un thème ») — seule forme de toute la page à ne pas être un anneau, alors que `/conversions` et `/couts` disent la même chose (une part d'un tout) avec `ThemeDonut` | la barre devient un anneau : `ThemeDonut` gagne une prop `carte?: boolean` (défaut `true`) qui, à `false`, rend l'anneau et sa légende SANS le cadre, le titre ni la note qu'il porte par défaut — pour se nicher dans une carte hôte qui les affiche déjà (ici, `labels-couverture` elle-même, dont le rang 1 est déjà le surtitre et le rang 4 déjà le verdict). Les trois usages existants restent des cartes autonomes, inchangés. Les deux parts sont des ÉTATS (rattaché / non rattaché), pas des thèmes : la teinte est FORCÉE (`TEINTE_COUVERTURE`, vert `#1a7a4a` / `NEUTRE` gris), même raisonnement que `TEINTE_CANAL` |
| `labels-couverture` (le camembert redevient un vrai camembert) | l'anneau opposait deux ÉTATS abstraits (rattaché / non rattaché), et il fallait deux paragraphes au-dessus pour l'expliquer — au point de devenir illisible (« 0 CHF... tout est rattaché » ne se comprend pas d'un coup d'œil). La vraie question posée par cette page n'est pas « est-ce rattaché » mais « où va mon argent, thème par thème » | une part PAR THÈME (`Couverture.parTheme`, calculé dans `getEtiquetage` par le même regroupement que `metaSansTheme`/`googleSansTheme`/`sansTheme.length` — dépense de la fenêtre de 90 jours, nombre d'éléments tous canaux confondus sur tout l'historique), plus une part « Sans thème » (teinte forcée `NEUTRE`, même raisonnement que l'ancien `TEINTE_COUVERTURE`) pour qu'elle ne disparaisse jamais silencieusement de l'écran. `ThemeDonut` gagne un champ optionnel `count` par ligne de `rows`, affiché dans la légende et l'info-bulle à côté du montant — les trois usages existants (`/`, `/couts`, `/conversions`) ne le passent pas et restent inchangés. Le rang 3/4 (chiffre + verdict binaire) et les deux paragraphes d'explication disparaissent : le camembert PORTE désormais son chiffre — le total et le nombre de thèmes, au centre de l'anneau (rang 6) — une seule phrase directe le précède |
| `labels-couverture` / `ThemeDonut` (dimensionner par le nombre, pas par le montant) | trois défauts trouvés à la relecture. (1) dimensionner par `spend` rend tout thème purement organique invisible par construction (0 CHF = surface nulle) — David : « on s'en fiche de la dépense » pour la taille des parts. (2) `dela.every(...)` sur un tableau VIDE vaut `true` (JS) : dès qu'il y a ≤ 5 lignes à montant positif et un `orphan > 0`, la part « autres » recevait `count: 0`, affiché « 0 élément » alors qu'elle en contenait plusieurs. (3) « Sans thème » restait une ligne ordinaire de `rows` : sur un compte à plus de 5 thèmes, elle pouvait se faire absorber dans le paquet générique « autres » — exactement ce que le module promet de ne jamais faire | `ThemeDonut` gagne `parNombre` (la taille des parts, le tri et le seuil « visible » suivent `count` au lieu de `spend` ; `orphan`, qui est un montant, est ignoré dans ce mode) et `epingles` (labels qui restent leur propre part quel que soit leur rang, jamais bucketés). `resteCount` exige `dela.length > 0` en plus du `every` — un tableau vide ne vaut plus un compte à zéro. Les trois usages existants ne passent ni l'un ni l'autre et dimensionnent par `spend` à l'identique (vérifié par simulation isolée du calcul, hors JSX, sur les quatre cas : bug d'origine, thème à 0 CHF visible, épingle non bucketée, usage existant inchangé). Le pied (rang 9) est réécrit pour dire les DEUX portées de la part : le nombre d'éléments porte tout l'historique, le montant CHF affiché à côté ne couvre que la fenêtre de 90 jours — les confondre dans un seul « tout ce module couvre 90 jours » aurait été un mensonge sur la moitié du chiffre |
| `labels-couverture` (le pied ne couvrait que le camembert) | la réécriture ci-dessus avait supprimé « publications comprises » sans la remplacer — cette précision scopait le rang 7 (le détail par canal, avec le nombre de publications Instagram sans thème, lui-même filtré par fenêtre dans `getEtiquetage`), pas seulement le camembert. Un lecteur du nouveau pied (« le nombre d'éléments porte tout l'historique ») appliquait cette même règle au chiffre du rang 7 juste en dessous, qui lui est scopé 90 jours — même défaut de classe que le rejet précédent, déplacé du camembert vers le rang 7. Trouvé au rejeu différentiel du checker (300k cas) | le pied nomme désormais les TROIS portées séparément : le nombre d'éléments du camembert (tout l'historique), son montant CHF (90 jours), et le détail par canal du rang 7 — CHF Meta/Google et nombre de publications Instagram — qui partage la fenêtre de 90 jours du montant, pas celle du nombre. **Règle : un pied qui corrige une confusion de portée doit vérifier CHAQUE chiffre qu'il couvre, pas seulement celui qui a motivé la réécriture** — une portée juste peut en démasquer une autre qu'on n'a pas encore nommée. Correction secondaire au passage : en mode `parNombre`, la légende affichait le CHF sans unité à côté d'un centre d'anneau qui déclare `uniteValeur="éléments"` — seul cas où le nombre nu de `montants` ne correspond pas à l'unité du centre. `parNombre` ajoute désormais « CHF » explicitement après le montant dans la légende, dans ce mode seulement |
| `Moyennes` / `MoyennesAds` / `MoyennesInsta` → tuiles `AdsKpis` / `InstaKpis` | *(2026-09-01, demande de David)* deux cartes empilées sur les trois pages canal — les KPIs de tête (période, avec delta) puis, juste dessous, une carte séparée « Tes moyennes » (par jour/mois/publication) — parlaient toutes deux de dépense/clics/CTR : lu comme un doublon, pas comme deux questions. Et sur Instagram, « Ta page » n'était même pas au format `Chiffre` NI À SA SPARKLINE (3 `<div>` à la main) — sa tuile Abonnés avait quand même un delta coloré fait main (vert/rouge, triangle), juste pas celui de `Chiffre`/`Pente` | **fusion dans la tuile, pas dans une carte à côté** : le calcul (`calculMoyennesAds` / `calculMoyennesInsta`, ex-`MoyennesAds`/`MoyennesInsta`, inchangé) nourrit un petit texte sous le delta de chaque tuile (`Chiffre.sous`) au lieu de sa propre boîte — le composant générique `Moyennes` disparaît. **CTR et CPC affichent DEUX chiffres, jamais un seul tranché** : le ratio-des-sommes sur la période (déjà celui d'`AdsKpis`, inchangé, rebaptisé « CTR/CPC période ») et la moyenne des ratios par jour/mois (déjà celle de l'ex-`Moyennes`, ex. « CTR moyen/jour ») ne mesurent pas la même chose et l'écart est déjà documenté (qa-testeur, CTR Google 7.38 % vs 4.96 %) — décision de David : les deux se lisent côte à côte sous des libellés distincts plutôt que d'en choisir un. **Un seul paragraphe d'honnêteté sous la grille**, pas un par tuile : mois incomplets écartés, jour sans campagne compté comme zéro MESURÉ, lien pour élargir la période quand la fenêtre est trop courte — les mêmes règles qu'avant, à un seul endroit. **`InstaKpis` (nouveau, puis scindé)** reprend les 3 tuiles de « Ta page » et ajoute une tuile PAR métrique de `calculMoyennesInsta` (portée, vues, j'aime, commentaires, enregistrements, engagement, publications) : pas de mapping partiel qui en laisserait de côté, donc une carte plus chargée que sur Meta/Google, assumé. « Engagement » est renommé « Engagement (moyenne) » sur sa tuile pour ne pas se lire comme le doublon d'« Engagement du compte », qui reste une moyenne d'historique. **Le delta absolu des abonnés (jamais un `%`, donc pas compatible avec `Pente`) garde son signal coloré** : `Chiffre` gagne `deltaNode?: ReactNode`, un remplacement du `<Pente>` automatique par un rendu fourni par l'appelant — la tuile Abonnés y repasse exactement l'ancien bloc (couleur par le sens, triangle), au lieu de le dégrader en texte gris uniforme dans `sous`. **Test humain non concluant sur cette première forme** (David, capture à l'appui) : une seule grille de 9 tuiles mélangeait des métriques DE PAGE (Abonnés, Croissance 30 j, Engagement du compte — l'état du compte) et des métriques DE PUBLICATION (`calculMoyennesInsta` — la moyenne d'un post sur la fenêtre) sans aucune séparation — « mélange pas page et métrics des postes ». `InstaKpis` est scindé en `InstaKpisPage` (les 3 tuiles de page, à côté de `CourbeAbonnes`) et `InstaKpisPosts` (les tuiles de publication, avec son propre titre « Tes moyennes par {unité} · n · fenêtre » — rang 1/2 que portait l'ancien module `Moyennes`, remonté ici puisque `InstaKpisPosts` n'a plus la compagnie du titre partagé « Ta page »). **Et les cartes doivent précéder le graphe qu'elles résument** : `InstaKpisPosts` (des métriques DE PUBLICATION) est déplacé juste AU-DESSUS de `PostsMetricChart` (le graphe par publication), au lieu d'être coincé entre `InstaKpisPage` et `CourbeAbonnes` — même ordre que Meta/Google, où `AdsKpis` précède `MetricChart`. **Troisième retour, même jour : « pas juste un texte »** — le `sous` textuel de la moyenne (« 12 CHF/jour en moyenne ») ne suffisait pas, David demandait « un sparkline avec une belle couleur pour les moyennes et en hover on voit le chiffre ». `Chiffre` gagne `serieMoyenne`/`serieMoyenneLabels`/`uniteMoyenne` : une SECONDE sparkline, empilée sous celle de la valeur (rang 6, deux formes au lieu d'une — la seule exception documentée à « une seule forme par module », parce que les deux disent deux choses différentes de la même tuile), couleur AMBRE FIXE (`TRAIT.warn`, désormais exporté depuis `chiffre.tsx` — la même couleur que le repère de seuil sur `LineChart`, jamais celle du `ton` de la tuile), bulle au survol identique à celle de la sparkline de valeur. **Elle trace `ChiffreMoyen.parUnite`, jamais `serie`** : la valeur du jour à côté de sa propre moyenne, pas la même courbe redessinée deux fois. Sur `InstaKpisPosts`, où le chiffre de tête EST déjà la moyenne (pas de « valeur de la période » distincte comme sur `AdsKpis`), une seule sparkline suffit — en `serieMoyenne`, jamais `serie`, pour ne pas dessiner deux fois la même donnée. **Complété « pour tous » (2e demande de David)** : les tuiles Impressions (hero), CPM et Portée n'avaient encore ni sparkline de valeur ni moyenne — `DayPoint` gagne un champ `reach` (absent jusque-là de la série journalière, `lib/channels.ts`), et `calculMoyennesAds` gagne deux entrées, CPM (taux, ratio-des-sommes comme CTR/CPC) et Portée (somme, comme Dépense/Clics/Impressions) : les 7 tuiles de coûts d'`AdsKpis` ont maintenant la paire complète (valeur + moyenne), pas 5 sur 7. **Trois défauts trouvés au rejeu du checker (3e passage), aucun retour humain cette fois** : (1) EN BRANCHE « JOUR » (le réglage par défaut, 7/14/30 jours), chaque unité moyennée EST un jour — `ChiffreMoyen.parUnite` y valait donc, point pour point, la même chose que `serie` : les deux sparklines empilées dessinaient la MÊME COURBE dans deux couleurs, contredisant le commentaire « jamais la même série ». Corrigé par `lisser()` (moyenne glissante sur 7 points, nuls ignorés dans la fenêtre) appliquée à `parUnite` SEULEMENT pour nourrir la sparkline de moyenne en branche jour — jamais pour le chiffre unique de `sous`, qui reste le calcul exact non lissé. En branche mois, `parUnite` (un mois entier par point) restait déjà distinct de `serie` : rien à lisser. (2) Sur Instagram, `d.posts` arrive trié date DESCENDANTE ; la branche « publication » (fenêtres courtes) en héritait telle quelle pour `serieMoyenne`/`labels`, donc lue récent→ancien, alors que la branche « mois » (`moisDeLaPeriode`, qui régénère l'ordre depuis la fenêtre) et `PostsMetricChart` juste en dessous (`.reverse()` explicite) lisent ancien→récent — le sens changeait selon le filtre choisi. `calculMoyennesInsta` trie désormais `posts` par date ascendante avant `uniteDeLaFenetre`, un seul sens dans les deux branches. (3) Le hero d'`AdsKpis` empilait ses deux sparklines avec `-mb-5` sur CHACUNE (la marge négative pensée pour annuler le padding de la carte sur le DERNIER élément) : la seconde remontait de 20 px et chevauchait la première. Un seul conteneur `-mx-5 -mb-5` autour des deux résout — elles s'empilent en flux normal à l'intérieur, sans marge négative propre à chacune |
