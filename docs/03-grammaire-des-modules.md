...
- `Comparer` — le réglage de comparaison ne survit pas à un clic sur la période,
  la métrique ou un filtre : `qs()` (pages pub) et les trois constructeurs de
  liens d'Instagram ne recopient pas `cmp` / `cfrom` / `cto`, et le module
  retombe sur « période précédente ». Rien de faux à l'écran — le module
  recalcule tout depuis l'URL — mais un réglage qu'on repose à chaque
  navigation.
