    ...
    with tab_perf:
        ...
        if "labels" in df.columns and not df.empty:
            ...
            if not real_labels.empty:
                ...
                # CSS inline pour les cartes label (Pulse style)
                st.markdown("""
                <style>
                .lbl-card {
                    background: #fff;
                ...
                # Identifier le best (meilleure portée moyenne parmi les vrais labels)
                best_lbl = real_labels.iloc[real_labels["reach_avg"].idxmax() - real_labels.index[0]]["_lbl"] \
                    if not real_labels.empty else None
                max_reach_avg = agg_lbl["reach_avg"].max() or 1

                # Afficher les cards en grille (2 colonnes)
