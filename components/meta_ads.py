...
.bud-bar-fill { display:block;height:100%;border-radius:99px;transition:width 0.3s; }
.bud-pacing-ok   { color:#1a7a4a;font-size:11.5px;font-weight:600; }
.bud-pacing-over { color:#c0392b;font-size:11.5px;font-weight:600; }
.bud-pacing-low  { color:#b86b00;font-size:11.5px;font-weight:600; }
.cost-grid {
...
def _show_budget_tab(df: pd.DataFrame | None) -> None:
    if df is None or (isinstance(df, pd.DataFrame) and df.empty):
    ...
    sel_period = st.radio("Période", list(period_opts.keys()), index=1, horizontal=True, key="bud_period")
    days = period_opts[sel_period]
    cutoff = pd.Timestamp(date.today() - timedelta(days=days))
    ...
    # ── Bloc 1 : KPI globaux + budget global ──────────────────────────────────
    st.markdown('<div class="bud-section-title">Vue globale des dépenses</div>', unsafe_allow_html=True)

    c1, c2, c3, c4 = st.columns(4)
    with c1:
        st.markdown(
            f'<div class="kpi-p"><div class="kp-lbl">Total dépensé</div>'
            f'<div class="kp-val">{total_spend:,.0f}<span class="kp-unit"> CHF</span></div></div>',
            unsafe_allow_html=True,
        )
    with c2:
        st.markdown(
            f'<div class="kpi-p"><div class="kp-lbl">Projeté 30j</div>'
            f'<div class="kp-val">{proj_30:,.0f}<span class="kp-unit"> CHF</span></div></div>',
            unsafe_allow_html=True,
        )
    with c3:
        st.markdown(
            f'<div class="kpi-p"><div class="kp-lbl">CPM moyen</div>'
            f'<div class="kp-val">{avg_cpm:.2f}<span class="kp-unit"> CHF</span></div></div>',
            unsafe_allow_html=True,
        )
    with c4:
        st.markdown(
            f'<div class="kpi-p"><div class="kp-lbl">CPC moyen</div>'
            f'<div class="kp-val">{avg_cpc:.2f}<span class="kp-unit"> CHF</span></div></div>',
            unsafe_allow_html=True,
        )
    ...
    budget_global = st.number_input(
        ...
        help="Saisis ton budget total pour la période pour voir ton taux de consommation",
    )
    if budget_global > 0:
        ...
        st.markdown(
            ...
            f'{total_spend:,.0f} CHF / {budget_global:,.0f} CHF — '
            f'<b>{pct*100:.0f}%</b> consommé</div>'
            f'<div class="bud-bar-wrap"><span class="bud-bar-fill" style="width:{pct*100:.1f}%;background:{bar_color};"></span></div>',
            unsafe_allow_html=True,
        )

    # ── Bloc 2 : Pacing par campagne ──────────────────────────────────────────
    st.markdown('<div class="bud-section-title">Pacing par campagne</div>', unsafe_allow_html=True)
    ...
    for col_h, lbl in zip(header, ["Campagne", "Label", "", "Budget max (CHF)", "Pacing"]):
        col_h.markdown(
    ...
    for _, row in df_camp.iterrows():
        ...
        c_name, c_label, _c_sep, c_budget, c_pacing = st.columns([3, 2, 1, 2, 2])

        with c_name:
            st.markdown(
                f'<div style="font-size:13px;font-weight:500;color:#0e0f12;padding-top:6px;">'
                f'{camp_name}</div>',
                unsafe_allow_html=True,
            )

        with c_label:
            st.text_input("Label", key=f"lbl_{safe_key}", label_visibility="collapsed",
                          placeholder="ex: Prospection Q2")
        ...
        with c_pacing:
            if bud_max > 0:
                ...
                if pct > 1:
                    status_html = '<span class="bud-pacing-over">⚠ Dépassé</span>'
                elif pct >= 0.7:
                    status_html = '<span class="bud-pacing-ok">✓ OK</span>'
                else:
                    status_html = '<span class="bud-pacing-low">↓ Sous-dépense</span>'
                st.markdown(
                    f'<div style="font-size:12px;color:#5a5d66;margin-top:6px;">'
                    f'{spend:,.0f} / {bud_max:,.0f} CHF {status_html}</div>'
                    f'<div class="bud-bar-wrap"><span class="bud-bar-fill" '
                ...
                st.markdown(
                    ...
                    f'{spend:,.0f} CHF dépensé</div>'
                    f'<div class="bud-bar-wrap"><span class="bud-bar-fill" '
                    f'style="width:0%;background:#e0e0e0;"></span></div>',
                    unsafe_allow_html=True,
                )

    # ── Bloc 3 : Coûts moyens ─────────────────────────────────────────────────
    st.markdown('<div class="bud-section-title">Coûts moyens</div>', unsafe_allow_html=True)
    ...
    metrics = [
        ...
        ("Fréquence", f"{freq:.2f}", "x", "Impressions / portée"),
    ...
    for col, (lbl, val, unit, tip) in zip(cost_cols, metrics):
        with col:
            st.markdown(
                f'<div class="kpi-p">'
                f'<div class="kp-lbl" title="{tip}">{lbl}</div>'
                f'<div class="kp-val">{val}<span class="kp-unit"> {unit}</span></div>'
                f'</div>',
                unsafe_allow_html=True,
            )
...
def show_meta_ads_tab(is_paid: bool = False):
    ...
    tab_perf, tab_budget = st.tabs(["Performance", "Budget"])
    with tab_perf:
