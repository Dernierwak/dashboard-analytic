...
|---|---|
| `test_moteur_unique.py` | Les deux règles du moteur (`_rule_format_gagnant`, `_rule_creneau`) n'existent plus — ni la fonction, ni leur clé dans les **cinq** tables de grammaire, ni leur mention dans la pondération par constat (`VISION_RULES`, lue sur l'arbre), ni les deux seuils qui ne servaient qu'à elles. **Et la relève sort** : `format_best` et `slot_best` sont toujours produits par `build_constats`, le créneau nommant son jour et son heure. Supprimer sans vérifier la relève retirerait la réponse au lieu d'unifier les moteurs. |
